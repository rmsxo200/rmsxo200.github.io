...
보통 조회시 불필요한 데이터를 전부 조회할 경우 성능적으로 안좋기 때문에 `Lazy`를 사용한다.  
...
> `post.getMember().getName()`는 실제 엔티티 객체를 참조만 할뿐 `proxy`객체가 초기화 되지는 않는다.  
...
```
조회된 `member`객체를 보면 놀랍게도 조회된 데이터가 아닌 `post`에서 조회된 `proxy`객체가 들어있다.  
