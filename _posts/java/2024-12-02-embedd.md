...
메핑엔티티는 자식 엔티티로 `@ManyToOne`로 부모 엔티티와 연결해 준다.  
...
위처럼 매핑테이블과 복합키를 만들었다면 이에 연결되는 부모 엔티티도 수정해 주어야 한다.  
...
`Member`에 옵션을 줬기 때문에 `Member`엔티티가 삭제되거나 혹은 리스트에서 제거된 `TrvlDstnMapping`는 자동으로 삭제된다.  
하지만 `Restaurant`에는 옵션을 주지 않았기 때문에 해당 엔티티를 삭제해도 `TrvlDstnMapping`는 삭제되지 않는다.  
...
근데 `CascadeType.ALL`와 `orphanRemoval = true` 옵션을 줬는데도 왜 안되는 이유가 궁금하긴하다;;;  
이거 왜이럼;;  
  
```
@Transactional
public void deleteMemberMain(long memberId) {
    Member member = memberRepository.findById(memberId)
            .orElseThrow(() -> new Exception("Member not found with memberId: " + memberId));
...
Restaurant restaurant = restaurantRepository.findById(restaurantId)
        .orElseThrow(() -> new Exception("Restaurant not found with restaurantId: " + restaurantId));
...
https://dev-elop.tistory.com/entry/JPA-orphanRemoval-용도
