...
## 코딩 규칙
- DTO는 Java record 사용 (class 금지)
- 응답 DTO에 `from(Entity)` 정적 팩토리 메서드 필수
- API 응답은 `ApiResponse<T>`로 통일, ResponseEntity 직접 사용 금지
- 생성자 주입만 사용 (@RequiredArgsConstructor)
- @Getter만 허용, @Data/@Setter 금지
- 엔티티 상태 변경은 도메인 메서드로 (cancel(), complete())
...
- given/when/then 구조, BDDMockito 사용
