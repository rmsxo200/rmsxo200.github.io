...
- 생성자 주입 + `@RequiredArgsConstructor`만 허용한다.
- 조회 메서드에 `@Transactional(readOnly = true)` 필수.
- 컨트롤러에 비즈니스 로직을 넣지 않는다. 서비스에 위임만 한다.
...
- 기능 코드 변경 시 테스트도 반드시 함께 변경. 예외 없음.
...
- `Optional`을 파라미터로 받는 것 (리턴 타입에만 허용)
- 문자열 비교에 `==` (→ `equals()` 또는 `Objects.equals()`)
```
...
> 💡 **팁**: Claude Code가 프로젝트에서 외부 가져오기를 처음 만나면 승인 대화를 표시합니다. 거부하면 해당 가져오기가 비활성화됩니다.
