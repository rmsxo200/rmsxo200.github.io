...
<span style="color: yellow">https://leetcode.com/problems/roman-to-integer/description/</span><br/>
<br/>
로마숫자 표
...
public int romanToInt(String s) {    
    ...
    for (int i = 0; i < s.length(); i++) {
        if(i != s.length()-1) {//마지막이 아닐 경우 비교
            ...
            if(thisNum < nextNum) { //다음숫자가 이전숫자보다 클 경우 두숫자를 한번에 계산
                sum += nextNum - thisNum;
                i++; //2개의 숫자를 한번에 사용했기에 1을 추가로 더 증가시킴
            } else {
                sum += thisNum;
            }
