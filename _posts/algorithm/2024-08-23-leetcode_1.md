...
<br/>
<br/>
시간 복잡도보다 O(n2) 작은 수정 코드<br/>(실행시간 : 59ms)
```
public int[] twoSum(int[] nums, int target) {
    for (int i = 0; i < nums.length; i++) {
...
<br/>
<br/>
시간 복잡도보다 O(n2) 작은 수정 코드2<br/>실행시간 : 439ms<br/>
&nbsp;&nbsp;&nbsp;<span style="color: red">* 코드의 라인수는 적으나 내부함수를 많이 사용하여 오히려 O(n2) 보다 속도 저하됨</span><br/>
```
public int[] twoSum(int[] nums, int target) {
...
<br/>
<br/>
시간 복잡도보다 O(n2) 작은 수정 코드3<br/>실행시간 : 4ms
```
public int[] twoSum(int[] nums, int target) {
