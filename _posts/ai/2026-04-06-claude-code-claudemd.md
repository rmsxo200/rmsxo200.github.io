...
> Subagent도 자신의 자동 메모리를 유지할 수 있다.  
> 자세한 내용은 [subagent 구성](https://code.claude.com/docs/ko/sub-agents#enable-persistent-memory)을 참조.  
  
---
  
### CLAUDE.md 파일의 위치  
`CLAUDE.md` 파일은 여러 위치에 있을 수 있으며 각각 다른 범위를 가진다.  
우선순위는 `settings.json`와 반대로 더 구체적인 위치가 더 광범위한 위치보다 우선한다.  
```
사용자 지침 > 프로젝트 지침 > 관리 정책
```  
...
/init
```  
> CLAUDE.md가 이미 존재하면 `/init`은 덮어쓰지 않고 개선 사항을 제안한다.  
...
* 일관성 : 충돌하는 규칙 제거  
  ...
  * 모노레포에서는 `claudeMdExcludes`를 사용하여 작업과 관련 없는 다른 팀의 CLAUDE.md를 건너뛸 수 있다.  

<br/>  

> **HTML 주석으로 메모 남기기**  
> CLAUDE.md 파일의 블록 수준 HTML 주석(`<!-- maintainer notes -->`)은 Claude의 컨텍스트에 로드될 때 자동 제거된다.  
> 토큰을 소비하지 않으면서 유지보수 담당자를 위한 메모를 남길 수 있는 유용한 기능이다.  
> 단, 코드 블록 안의 주석은 제거되지 않고 그대로 보존된다.  
...
> 거부하면 해당 가져오기가 비활성화된 상태로 유지되고 대화가 다시 나타나지 않는다.  
  
<br/>  
   
#### 가져오기의 기술적 세부사항  
- 상대 경로는 **작업 디렉토리가 아닌, 가져오기를 포함하는 파일 기준**으로 해석된다.
- 가져온 파일이 다시 다른 파일을 가져올 수 있다 (최대 **5단계 깊이**까지).
- `@README.md`처럼 파일명만 쓰면 CLAUDE.md와 같은 디렉토리에서 찾는다.
- `@~/`로 시작하면 홈 디렉토리를 기준으로 한다.
  
<br/>  
   
#### AGENTS.md와 공존하기  
다른 코딩 에이전트(Cursor, Windsurf 등)를 위한 `AGENTS.md`가 이미 있다면, CLAUDE.md에서 가져오기만 하면 된다.  
Claude Code는 `AGENTS.md`를 직접 읽지 않으므로 반드시 `@`로 가져와야 한다.  
```markdown
# CLAUDE.md
...
Claude Code는 현재 작업 디렉토리에서 디렉토리 트리를 따라 올라가며 CLAUDE.md 파일을 읽고 각 디렉토리를 확인한다.  
예를 들어 `foo/bar/`에서 Claude Code를 실행하면 `foo/bar/CLAUDE.md`와 `foo/CLAUDE.md` 모두에서 지침을 로드한다.  

또한 현재 작업 디렉토리 아래의 하위 디렉토리에서도 CLAUDE.md 파일을 발견한다.  
이 경우 시작 시 로드하는 것이 아니라 Claude가 해당 하위 디렉토리의 파일을 읽을 때 포함된다.  

대규모 모노레포에서 다른 팀의 CLAUDE.md 파일이 선택되는 경우 `claudeMdExcludes`를 사용하여 건너뛸 수 있다.  

#### 추가 디렉토리에서 로드  
`--add-dir` 플래그는 주 작업 디렉토리 외부의 추가 디렉토리에 대한 액세스를 제공한다.  
기본적으로 이러한 디렉토리의 CLAUDE.md 파일은 로드되지 않는다.  
추가 디렉토리에서 CLAUDE.md를 로드하려면 환경 변수를 설정한다:  
```bash
...
> 규칙은 모든 세션 또는 일치하는 파일이 열릴 때 컨텍스트에 로드된다.  
> 항상 컨텍스트에 있을 필요가 없는 **작업별 지침**은 rules 대신 [skills](https://code.claude.com/docs/ko/skills)를 사용하자.  
...
`.claude/CLAUDE.md`와 동일한 우선순위로 시작 시 로드된다.  
...
이 규칙은 Claude가 `OrderController.java`나 `MemberApiController.java` 같은 파일을 읽을 때만 컨텍스트에 로드된다.  
`OrderService.java`를 작업할 때는 로드되지 않아 불필요한 토큰 소비를 줄인다.  

> **참고**: 경로별 규칙은 모든 도구 사용 시가 아니라 Claude가 패턴과 일치하는 **파일을 읽을 때** 트리거된다.  
...
심볼릭 링크는 해석되어 정상적으로 로드되며, 순환 심볼릭 링크는 감지되어 우아하게 처리된다.  
```bash
# 공유 규칙 디렉토리 전체를 링크
...
#### 특정 CLAUDE.md 파일 제외하기 (claudeMdExcludes)  
대규모 모노레포에서 상위 CLAUDE.md 파일에 작업과 관련 없는 지침이 포함될 수 있다.  
`claudeMdExcludes` 설정으로 경로 또는 glob 패턴으로 특정 파일을 건너뛸 수 있다.  
  
`.claude/settings.local.json` (로컬에만 유지):  
```json
{
  "claudeMdExcludes": [
    "**/monorepo/CLAUDE.md",
...
> 패턴은 glob 구문을 사용하여 절대 파일 경로와 일치한다.  
> `claudeMdExcludes`는 사용자, 프로젝트, 로컬, 관리 정책 설정에서 구성할 수 있으며 배열은 레이어 전체에서 병합된다.  
> 단, 관리 정책 CLAUDE.md 파일은 제외할 수 없다.  
...
자동 메모리를 통해 Claude는 직접 작성하지 않아도 세션 간에 지식을 축적할 수 있다.  
Claude는 작업하면서 스스로 노트를 저장한다: 빌드 명령, 디버깅 인사이트, 아키텍처 노트, 코드 스타일 선호도, 워크플로우 습관 등.  
매 세션마다 반드시 저장하는 것은 아니고, 향후 대화에서 유용할지 여부를 판단하여 기억할 가치가 있을 때만 저장한다.  

> 자동 메모리는 Claude Code v2.1.59 이상이 필요하다. `claude --version`으로 버전을 확인.  

#### 자동 메모리 활성화/비활성화  
자동 메모리는 기본적으로 켜져 있다. 토글 방법:  
- 세션에서 `/memory`를 열고 자동 메모리 토글 사용  
- 프로젝트 설정에서 `"autoMemoryEnabled": false` 설정  
- 환경 변수 `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` 설정  

#### 저장소 위치  
각 프로젝트의 자동 메모리는 `~/.claude/projects/<project>/memory/`에 저장된다.  
`<project>` 경로는 git 저장소에서 파생되므로, 동일한 저장소 내의 모든 worktree와 하위 디렉토리는 하나의 자동 메모리 디렉토리를 공유한다.  
git 저장소 외부에서는 프로젝트 루트가 대신 사용된다.  

```
~/.claude/projects/<project>/memory/
├── MEMORY.md          # 간결한 인덱스, 모든 세션에 로드됨 (처음 200줄)
├── debugging.md       # 디버깅 패턴에 대한 자세한 노트
├── api-conventions.md # API 설계 결정
└── ...                # Claude가 만드는 다른 주제 파일
```

> 자동 메모리 저장 위치를 변경하려면 사용자 또는 로컬 설정에서 `"autoMemoryDirectory": "~/my-custom-memory-dir"`를 설정한다.  
> 단, 보안상의 이유로 프로젝트 설정(`.claude/settings.json`)에서는 이 설정이 허용되지 않는다.  

#### 작동 방식  
- `MEMORY.md`의 **처음 200줄**만 모든 대화 시작 시 로드된다 (200줄 초과분은 로드되지 않음).  
- Claude는 자세한 노트를 별도의 주제 파일로 이동하여 `MEMORY.md`를 간결하게 유지한다.  
- `debugging.md`, `patterns.md` 같은 주제 파일은 시작 시 로드되지 않는다. Claude가 필요할 때 파일 도구로 읽는다.  
- Claude Code 인터페이스에서 "Writing memory" 또는 "Recalled memory"가 표시되면 Claude가 메모리 파일을 업데이트하거나 읽고 있는 것이다.  

> 이 200줄 제한은 `MEMORY.md`에만 적용된다.  
> CLAUDE.md 파일은 길이에 관계없이 전체 로드되지만, 더 짧은 파일이 더 나은 준수를 보여준다.  

#### 메모리 감사 및 편집  
자동 메모리 파일은 언제든지 편집하거나 삭제할 수 있는 일반 마크다운이다.  
`/memory`를 실행하면 세션 내에서 메모리 파일을 찾아보고 편집기에서 열 수 있다.  

Claude에게 "항상 npm이 아닌 pnpm을 사용해"라고 말하면 자동 메모리에 저장된다.  
CLAUDE.md에 직접 추가하고 싶다면 "이것을 CLAUDE.md에 추가해"라고 명시적으로 요청하면 된다.  
  
---
  
### 대규모 팀을 위한 CLAUDE.md 관리

조직에서 Claude Code를 팀 전체에 배포하는 경우, 중앙 집중식으로 관리하고 로드되는 CLAUDE.md 파일을 제어할 수 있다.

#### 조직 전체 CLAUDE.md 배포  
관리 정책 위치에 파일을 생성하고 MDM, 그룹 정책, Ansible 등으로 개발자 컴퓨터 전체에 배포한다.  
이 파일은 개별 설정으로 제외할 수 없으므로 조직 전체 지침이 항상 적용된다.  

#### 관리 정책 CLAUDE.md vs 관리 설정  
둘은 다른 목적을 가진다. 기술적 강제가 필요하면 설정을, 행동 지침은 CLAUDE.md를 사용한다.  
...
> 설정 규칙은 Claude의 결정과 무관하게 클라이언트에 의해 강제된다.  
> CLAUDE.md 지침은 Claude의 행동을 형성하지만 하드 강제 레이어가 아니다.  
  
---
  
### 문제 해결
1. **Claude가 CLAUDE.md를 따르지 않을 때**  
   * `/memory`를 실행하여 파일이 실제로 로드되는지 확인한다. 파일이 나열되지 않으면 Claude가 볼 수 없다.  
   * CLAUDE.md가 올바른 위치에 있는지 점검한다.  
   * 지침을 더 구체적으로 수정한다. "2칸 들여쓰기 사용"이 "코드를 제대로 포맷합니다"보다 잘 작동한다.  
   * 여러 파일에 걸쳐 충돌하는 규칙이 없는지 검토한다.  
   * 시스템 프롬프트 수준이 필요하다면 [`--append-system-prompt`](https://code.claude.com/docs/ko/cli-reference#system-prompt-flags)를 사용한다. 모든 호출 시 전달해야 하므로 대화형 사용보다는 스크립트/자동화에 더 적합하다.  
   * [`InstructionsLoaded` hook](https://code.claude.com/docs/ko/hooks#instructionsloaded)을 사용하면 로드된 지침 파일, 시기, 이유를 정확히 기록할 수 있어 디버깅에 유용하다.  
  
2. **자동 메모리가 저장한 내용을 모를 때**  
   * `/memory`를 실행하고 자동 메모리 폴더를 선택하여 Claude가 저장한 내용을 찾아본다.  
   * 모든 것이 읽고, 편집하거나 삭제할 수 있는 일반 마크다운이다.  
  
3. **`CLAUDE.md`가 너무 클 때**  
   * 200줄을 초과하면 `@path` 가져오기로 별도 파일을 참조하거나, `.claude/rules/`로 분할한다.  
  
4. **`/compact` 후 지침이 사라진 것 같을 때**  
   * `CLAUDE.md`는 압축 후에도 디스크에서 다시 읽혀 완전히 유지된다.  
   * 사라진 지침이 있다면 대화에서만 전달하고 `CLAUDE.md`에 기록되지 않은 것이다. 세션 간에 지속되도록 `CLAUDE.md`에 추가하자.  
  
---

### 마치며
`CLAUDE.md` 파일과 `skills`를 작성하는데 정답은 없다.  
...
이렇듯 정답은 없으므로 써보면서 나에게 맞게 조정해보자.  
