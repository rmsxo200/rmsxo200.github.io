...
- Sub-agent는 **다른 Sub-agent를 생성할 수 없다.** 항상 1단계 깊이로만 동작한다. 단, `claude --agent`로 주 스레드 에이전트로 실행되는 경우에는 `Agent(agent_type)` 구문을 통해 허용된 유형의 Sub-agent를 생성할 수 있다.  
...
그 외에도 특정 작업을 위한 도우미 에이전트가 있다. 일반적으로 자동 호출되므로 직접 사용할 일은 거의 없다.
...
Sub-agent는 이 시스템 프롬프트만 받으며(작업 디렉토리 등 기본 환경 정보 포함), 전체 Claude Code 시스템 프롬프트는 받지 않는다.
...
| `model` | | `sonnet`, `opus`, `haiku`, 전체 모델 ID(예: `claude-opus-4-6`), 또는 `inherit`(기본값) |
| `permissionMode` | | 권한 모드: `default`, `acceptEdits`, `auto`, `dontAsk`, `bypassPermissions`, `plan` |
| `maxTurns` | | Sub-agent가 중지되기 전의 최대 에이전트 턴 수 |
| `memory` | | 영속 메모리 범위: `user`, `project`, `local` |
| `background` | | `true`면 항상 백그라운드에서 실행 |
| `effort` | | 노력 수준: `low`, `medium`, `high`, `max`(Opus 4.6만 해당). 세션 노력 수준을 재정의 |
| `isolation` | | `worktree`로 설정하면 임시 git worktree에서 격리 실행 |
| `color` | | 작업 목록에서의 표시 색상: `red`, `blue`, `green`, `yellow`, `purple`, `orange`, `pink`, `cyan` |
| `initialPrompt` | | 주 세션 에이전트로 실행 시(`--agent`) 첫 번째 사용자 턴으로 자동 제출 |
| `hooks` | | 라이프사이클 훅 정의 |
| `skills` | | 시작 시 로드할 스킬 목록. 전체 콘텐츠가 컨텍스트에 주입됨 (부모 대화의 스킬은 상속되지 않음) |
...
- **플러그인** Sub-agent는 보안상 `hooks`, `mcpServers`, `permissionMode` 필드가 지원되지 않으며 로드 시 무시된다.
...
`@`를 입력하면 typeahead에서 Sub-agent를 선택할 수 있다. 수동 입력 시에는 `@agent-<name>` 형식을 사용한다.

```
@"code-reviewer (agent)" 인증 모듈 변경사항을 봐줘
```

> 전체 메시지는 Claude로 전달되며, Claude가 요청 내용을 기반으로 Sub-agent의 작업 프롬프트를 작성한다. @멘션은 Claude가 **어떤 Sub-agent를 호출할지**를 제어하며, Sub-agent가 받는 프롬프트는 제어하지 않는다.
...
이 모드에서는 Sub-agent의 시스템 프롬프트가 기본 Claude Code 시스템 프롬프트를 **완전히 대체**한다.  
프로젝트의 모든 세션에 기본 적용하려면 `.claude/settings.json`에 설정할 수도 있다.
...
### Foreground와 Background 실행

Sub-agent는 **foreground**(차단) 또는 **background**(동시) 모드로 실행할 수 있다.

- **Foreground**: 완료될 때까지 메인 대화가 차단된다. 권한 프롬프트나 질문이 사용자에게 전달된다.
- **Background**: 메인 대화와 동시에 실행된다. 시작 전에 필요한 도구 권한을 미리 요청하며, 실행 중에는 사전 승인되지 않은 항목을 자동 거부한다.

Claude가 상황에 따라 foreground/background를 결정하지만, 직접 제어할 수도 있다.
...
메모리가 활성화되면 `MEMORY.md`의 처음 200줄 또는 25KB(**둘 중 먼저 도달하는 것**)가 자동으로 시스템 프롬프트에 포함된다.  
해당 한계를 초과하면 `MEMORY.md`를 큐레이션하도록 지침이 함께 포함된다.  
또한 Read, Write, Edit 도구가 자동으로 활성화되어 Sub-agent가 메모리 파일을 관리할 수 있다.
...
> 주의: 각 Sub-agent가 완료되면 결과가 메인 대화로 반환된다. 각각 자세한 결과를 반환하는 많은 Sub-agent를 실행하면 상당한 컨텍스트를 소비할 수 있다.
...
| 빠른 수정이 필요할 때 | 메인 대화 |
| 이미 대화에 있는 내용에 대한 빠른 질문 | `/btw` (도구 접근 없이 전체 컨텍스트 참조) |
 
Sub-agent는 시작할 때 컨텍스트가 깨끗하기 때문에 정보를 다시 수집하느라 **지연이 생길 수 있다.**  
이미 맥락이 충분한 경우라면 메인 대화에서 바로 처리하는 게 빠르다.

> 격리된 컨텍스트가 아닌 메인 대화 컨텍스트에서 실행되는 재사용 가능한 프롬프트가 필요하다면 [Skills](https://code.claude.com/docs/ko/skills)를 고려하자.
